...
tape('git secure tag', (t) => {
  ...
  // Verify tags
  t.doesNotThrow(() => cmd(node, [cli, '--insecure', '-v', 'tag-latest']), 'valid HEAD evtag');
  t.doesNotThrow(() => cmd(node, [cli, '--insecure', '-v', 'tag-middle']), 'valid non-HEAD evtag');
  t.doesNotThrow(() => cmd(node, [cli, '--insecure', '-v', 'tag-legacy']), 'valid legacy hash');
