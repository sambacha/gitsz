...
Hash.prototype._visit = function _visit (state, hash, callback) {
  ...
  stream.on('header', (type, size) => {
    if (type !== 'tag') state.update(type + ' ' + size + '\0')

    const chunks = []
    stream.on('data', (data) => {
      if (type !== 'tag') state.update(data)
    ...
    stream.on('end', () => {
      if (type === 'blob') return callback(null)

      const content = Buffer.concat(chunks)

      if (type === 'tree') this._visitTree(state, content, callback)
      else if (type === 'commit') { this._visitCommit(state, hash, content, callback) } else if (type === 'tag') this._visitTag(state, hash, content, callback)
...
Hash.prototype._visitTree = function _visitTree (state, content, callback) {
  ...
  async.forEachSeries(
    ...
    (node, callback) => {
      ...
      if (node.mode === '160000') { return this._visitSubmodule(subState, node.hash, callback) }

      this._visit(subState, node.hash, callback)
    },
    (err) => {
      if (err && !/while loading/.test(err.message)) { err.message += `\nwhile loading tree at ${state.dir || '.'}` }
      callback(err)
    }
  )
}

Hash.prototype._visitCommit = function _visitCommit (
  state,
  hash,
  content,
  callback
...
Hash.prototype._visitSubmodule = function _visitSubmodule (
  state,
  hash,
  callback
) {
