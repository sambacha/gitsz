...
Hash.prototype._visit = function _visit(state, hash, callback) {
  ...
  stream.on('header', (type, size) => {
    ...
    stream.on('end', () => {
      ...
      else if (type === 'commit') this._visitCommit(state, hash, content, callback);
      else if (type === 'tag') this._visitTag(state, hash, content, callback);
...
Hash.prototype._visitSubmodule = function _visitSubmodule(state, hash, callback) {
  ...
  this._visit(moduleState, hash, (err) => {
    if (err && !/while loading/.test(err.message)) {
      err.message += '\nwhile loading submodule at ' + `${path.relative(state.root, moduleRoot)}`;
    }
