...
function buffer(stream) {
  ...
  return (callback) => {
    if (done) return callback(null, chunks);
    else stream.once('end', () => callback(null, chunks));
  };
}
...
API.prototype.verify = function verify(tag, options, callback) {
  ...
  const args = options.insecure ? ['show', tag] : ['verify-tag', '-v', tag];
  ...
  async.waterfall(
    [
      ...
      (p, callback) => {
        if (p.status !== 0) {
          if (callback) return callback(new Error(p.stderr.toString()), false);
        ...
        if (match === null) {
          const msg =
            'error: No `Git-EVTag-v0-SHA512: ...`, nor ' +
            '`Git-Secure-Tag-V0` found in tag description';
          if (callback) return callback(new Error(msg), false);
      ...
      (hash, match, p, callback) => {
        ...
        const msg =
          `error: Git-${match[1]} hash mismatch\n` +
          `error: Expected: ${hash}\n` +
          `error: Found: ${match[2]}`;
...
API.prototype.sign = function sign(tag, ref, options, callback) {
  ...
  async.waterfall(
    [
      ...
      (hash, callback) => {
        const name = options.legacy ? 'Git-Secure-Tag-V0' : 'Git-EVTag-v0-SHA512';
        const msg = (options.m || tag + '\n') + '\n' + `${name}: ${hash}\n`;

        let args = ['tag'];
        args.push('-m', msg);
        if (options.u) args.push('-u', options.u);
        else if (!options.insecure) args.push('-s');
